...
После создания переменной, в которой будут храниться опции (настройки) запуска, можно ее заполнить:
```python
...
- `--remote-debugging-port=9222` - этот флаг добавляет дебаг порт для удаленного подключения. Иногда спасает от некоторых ошибок Selenium и  
